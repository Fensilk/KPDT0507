"""
门控融合训练脚本（E5: ViT-g + Δframe 与姿态/光流的门控融合）。

与 train_phase6.py 的区别：
  - 模型用 GatedFusionModel（主特征 + 辅助特征分别分支，门控融合）
  - 数据加载 return_aux_separately=True（aux 单独返回，不 concat）

用法：
    python experiments/train_gated_fusion.py \
      --bridge --use_diff \
      --pose_npz data/omnifall_pose_frame.npz \
      --window_size 64 --stride 8 \
      --exp_tag phase8/p8_gated_pose
"""
import os
import sys
import json
...
def parse_args():
    parser = argparse.ArgumentParser(description="Gated fusion training")
    ...
    parser.add_argument("--pose_npz", type=str, default=None,
                        help="Aux NPZ (pose/flow) for gated fusion")
...
def main():
    ...
    if args.bridge:
        if args.exp_tag is None:
            args.exp_tag = "phase8/gated_fusion"
    if args.exp_tag:
        log_dir = f"logs/{args.exp_tag}"
    else:
        log_dir = "logs/phase8/gated_fusion"
    ...
    print(f"\n[INFO] main_dim={main_dim} (ViT-g + diff), aux_dim={aux_dim} (pose/flow)")
    ...
    print(f"[INFO] GatedFusionModel params: {n_params:,}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch:3d} | Train Loss {train_loss:.4f} | Val Loss {val_metrics['loss']:.4f} | "
              ...
              f"Avg F1 {val_metrics['avg_f1']:.4f} | LR {current_lr:.2e}")
    ...
    plot_confusion_matrix(cm, TERNARY_CLASS_NAMES,
                          ...
                          title=f"Gated Fusion — Avg F1={test_metrics['avg_f1']:.3f}",
                          normalize=True)
