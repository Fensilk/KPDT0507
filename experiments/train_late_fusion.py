"""
后期融合训练脚本（E6: ViT-g + Δframe 与姿态/光流的 late fusion）。

与 train_gated_fusion.py 的区别：模型用 LateFusionModel
（两个分支独立过 decoder，logit 层 α 加权融合）。

用法：
    python experiments/train_late_fusion.py \
      --bridge --use_diff \
      --pose_npz data/omnifall_optical_flow.npz \
      --window_size 64 --stride 8 \
      --exp_tag phase8/p8_late_flow
"""
import os
import sys
import json
...
def parse_args():
    ...
    parser.add_argument("--frame_npz", type=str, default="data/omnifall_dinov2_giant_frame.npz")
    parser.add_argument("--splits_dir", type=str, default="DATASET-omnifall/splits/syn/random")
    parser.add_argument("--window_size", type=int, default=64)
    parser.add_argument("--stride", type=int, default=8)
    parser.add_argument("--hidden_dim", type=int, default=384)
    parser.add_argument("--aux_hidden", type=int, default=128)
    parser.add_argument("--num_heads", type=int, default=6)
    parser.add_argument("--dropout", type=float, default=0.3)
    parser.add_argument("--bridge", action="store_true")
    parser.add_argument("--use_diff", action="store_true")
    parser.add_argument("--pose_npz", type=str, default=None, help="Aux NPZ (pose/flow)")
...
def main():
    ...
    if args.exp_tag:
        ...
        log_dir = "logs/phase8/late_fusion"
    ...
    print(f"\n[INFO] main_dim={main_dim}, aux_dim={aux_dim}")

    model = LateFusionModel(
        main_dim=main_dim, aux_dim=aux_dim,
        hidden_dim=args.hidden_dim, aux_hidden=args.aux_hidden,
        num_heads=args.num_heads, aux_num_heads=4, num_layers=1,
        dropout=args.dropout, max_len=args.window_size + 10, num_classes=3,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[INFO] LateFusionModel params: {n_params:,}")
    ...
    history = {"train_loss": [], "val_loss": [], "ternary_acc": [], "fall_f1": [],
               "fallen_f1": [], "normal_f1": [], "avg_f1": [], "lr": [], "alpha": []}
    best_fall_f1 = 0.0
    best_epoch = 0
    patience_counter = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch:3d} | Train Loss {train_loss:.4f} | Val Loss {val_metrics['loss']:.4f} | "
              ...
              f"Avg F1 {val_metrics['avg_f1']:.4f} | α={model.alpha:.3f}")
    ...
    plot_confusion_matrix(cm, TERNARY_CLASS_NAMES,
                          ...
                          title=f"Late Fusion — Avg F1={test_metrics['avg_f1']:.3f}",
                          normalize=True)
    ...
    results_to_save["config"] = {
        ...
        "aux_hidden": args.aux_hidden, "num_layers": 1,
        ...
        "best_epoch": best_epoch, "final_alpha": history["alpha"][-1] if history["alpha"] else 0.5,
    }
